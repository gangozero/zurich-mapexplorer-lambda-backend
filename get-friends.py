...
def lambda_handler(event, context):
    ...
    if user:
        if user['token'] == event['token']:
            answer = []
            if event['self']:
                rec = {'id': user['id'], 'name': user['name'], 'color': user['color'], 'xp': user['xp']}
                answer.append(rec)
            for friend in user['friends']:
                resp_friend = dynamo_request(dyn_users, {'id': friend})
                if resp_friend:
                    rec = {'id': resp_friend['id'],
                           'name': resp_friend['name'],
                           'color': resp_friend['color'],
                           'xp': resp_friend['xp']}
                    answer.append(rec)
                else:
                    raise Exception("Bad Request: Error quering friends data")
            return answer
